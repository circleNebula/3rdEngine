...
private:
	...
	unsigned int*	textureIds;							// pointer to texture Array
	...
	void apply_material(const aiMaterial *mtl)
	{
		...
		if(AI_SUCCESS == mtl->GetTexture(aiTextureType_DIFFUSE, texIndex, &texPath))
		{
			...
			for(int i = 0;i < numTextures; i++){
				//cout << texPath.data << "::"<< sFilename[i] << endl;
				if(std::strcmp(texPath.data, (sFilename[i].c_str())) == 0){
					//cout << "binding" << texPath.data << endl;
...
public:
	...
	void recursive_render (const struct aiScene *sc, const struct aiNode* nd, float scale, bool shadow)
	{
		unsigned int i;
		unsigned int n=0, t;
...
*/
	//		glBegin(GL_TRIANGLES);
		...
		{
			recursive_render(sc, nd->mChildren[n],scaleF, shadow);
		}
	...
	int DrawGLScene(bool shadow)				//Here's where we do all the drawing
	{		
...
//glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	// Clear The Screen And The Depth Buffer
	...
	//glTranslatef(0,-10,0);
		...
		recursive_render(_scenes[0], _scenes[0]->mRootNode, scaleF, shadow);
	...
	//	ilInit(); /* Initialization of DevIL */
		...
		textureIds =  (unsigned int*) calloc (numTextures,sizeof(unsigned int));
		...
		for (int i=0; i<numTextures; i++)
		{
			...
			success = -1;//loadTexture(i, fileloc.c_str());
			if (success == -1)
			{
				std::string::size_type v = filename.find('\\');
				while (v != std::string::npos)
				{
					filename.erase(0, v + 1);
					v = filename.find('\\');
				}
				fileloc = filename;
				
				success = loadTexture(i, fileloc.c_str());
				//sFilename[i] = fileloc;
			}

			if (success != -1) /* If no error occured: */
			{
				//cout << fileloc <<"<- loaded texture" <<endl;
				textures[i] = success;
				//cout << i << "< i" << endl;
	...
	unsigned int loadTexture(int number, const char* filename){
		
	   // glGenTextures(number, &texture);
		glBindTexture(GL_TEXTURE_2D, textureIds[number]); // all upcoming GL_TEXTURE_2D operations now have effect on this texture object
		...
		if (data){
			...
			cout << "loadedTexure "<< filename << endl;
		}else{
			std::cout << "Failed to load texture"  << std::endl;
			return -1;
		}	
		stbi_image_free(data);
	//	cout << "loadedTexure "<< textureIds[number] << endl;
		return textureIds[number];
	}
